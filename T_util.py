...
def folderV2Seg(Do,dt=np.uint16,step=1):
    from scipy.misc import imread
    import glob
    fns = sorted(glob.glob(Do+'*.png'))
    sz = imread(fns[0])[::step,::step].shape
    seg = np.zeros((len(fns),sz[0],sz[1]), dtype=dt)
    if dt == np.uint16:
        for zi in range(len(fns)):
            seg[zi] = vast2Seg(imread(fns[zi]))[::step,::step]
    else:
        for zi in range(len(fns)):
            tmp = imread(fns[zi])
            if tmp.ndim==3:
...
def bfly_cv(bfly_db, x0, x1, y0, y1, z0, z1, tile_sz, dt=np.uint8,st=1, tile_ratio=1, resize_order=1):
    ...
    return result
def bfly(bfly_db, x0, x1, y0, y1, z0, z1, tile_sz, dt=np.uint8, tile_st=[0,0], tile_ratio=1, resize_order=1):
    # x: column
    # y: row
    # no padding at the boundary
    # st: starting index 0 or 1
...
def write_bfly(sz, numT, imN, tsz=1024, tile_st=[0,0],zPad=[0,0], im_id=None, outName=None,st=0):
...
def readh5_b(filename, sz, datasetname='main'):
    import h5py
    tmp = np.array(h5py.File(filename,'r')[datasetname])
    return tmp[:np.prod(sz)].reshape(sz)

def writeh5_b(filename, dtarray, datasetname='main'):
    # for binary data
    import h5py
    fid=h5py.File(filename,'w')
    if isinstance(datasetname, (list,)):
